    ...
    >
      <nav className="mx-auto flex h-[72px] max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
        ...
        <Link href="/" className="group flex items-center gap-3">
          <span className="relative block h-11 w-11 overflow-hidden rounded-2xl border border-white/15 bg-ink-800 shadow-[0_0_30px_-6px_rgba(255,61,138,0.5)] transition-transform group-hover:scale-105">
          ...
          <span className="flex flex-col leading-none">
            <span className="flex items-center gap-2">
              ...
              <span className="hidden rounded-full border border-brand/30 bg-brand/10 px-2 py-0.5 font-mono text-[9px] font-bold uppercase tracking-wider text-brand sm:inline">
                SaaS Engine
              </span>
            </span>
        ...
        <div className="hidden items-center gap-2.5 md:flex">
          <span className="flex items-center gap-2 rounded-lg border border-white/10 bg-white/[0.03] px-2.5 py-1.5 font-mono text-[10px] font-semibold uppercase tracking-wider text-slate-400">
            <span
              className={online === false ? 'text-red-400' : 'text-emeraldx'}
          ...
          {user ? (
            <>
              {user.role === 'admin' && (
                <Link
                  href="/admin"
                  className="btn h-9 gap-1.5 rounded-lg border border-red-500/40 bg-red-500/10 px-3 text-xs font-bold text-red-400 transition hover:bg-red-500/20"
                >
              ...
              <Link
                href="/dashboard"
                className="btn h-9 gap-2 rounded-lg border border-brand/40 bg-brand/15 px-3.5 text-xs font-bold text-brand transition hover:bg-brand/25"
              >
            ...
            <>
              <Link
                href="/auth/login"
                className="rounded-lg px-3 py-2 text-[13px] font-semibold text-slate-300 transition-colors hover:text-white"
              >
                Войти
              </Link>
            ...
            {user ? (
              <>
                {user.role === 'admin' && (
                  <Link
                    href="/admin"
                    className="btn h-11 rounded-xl border border-red-500/40 bg-red-500/10 text-sm font-bold text-red-400"
                  >
                ...
                <button onClick={logout} className="py-2 text-center text-sm font-semibold text-red-400">
                  Выйти из системы
                </button>
              </>
            ) : (
