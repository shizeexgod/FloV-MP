...
public static class AltvClientCore
{
    ...
    public static string DetectPlatform(string gtaPath)
    {
        ...
        if (Has("EOSSDK-Win64-Shipping.dll") || Has("EOSSDK-Win64-Shipping-1.17.1.3.dll") || Has("Rockstar-Games-Epic.exe") || HasDir(".egstore") || Has("title.rgl")) return "rgl";
        if (Has("steam_api64.dll") || HasDir("steamapps")) return "steam";
        if (Has("PlayGTAV.exe") && Has("GTAVLauncher.exe") && !Has("steam_api64.dll")) return "rgl";

        var path = gtaPath.ToLowerInvariant();
        if (path.Contains("\\epic games\\") || path.Contains("epicgames") || path.Contains("9d2d0eb64d5c44529cece33fe2a46482")) return "rgl";
