...
rem
rem  The genuine 16.4.39 dll is the one already on this PC in the alt:V payload
rem  and verified against the official 16.4.39 update manifest.
...
for /f "skip=1 tokens=1" %%H in ('certutil -hashfile "%SRC%" SHA1') do (
...
for %%F in ("%RC%\altv-client.dll" "%RC%\altv-client.dll.orig.bak" "%RC%\patched\altv-client.dll") do (
    set "H="
    for /f "skip=1 tokens=1" %%H in ('certutil -hashfile "%%~F" SHA1') do if not defined H set "H=%%H"
    echo   !H!  %%~F
)
