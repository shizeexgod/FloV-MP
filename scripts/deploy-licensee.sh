...
#
# Формат сверен с живой рабочей установкой, а не подобран по строкам бинарника.
# Ключевые детали, на которых легко ошибиться:
#   * externalSecret — ЧИСЛО без кавычек, совпадает с secret в voice.toml;
#   * портов ДВА: externalPort (игровой <-> голосовой, наружу не нужен) и
#     externalPublicPort (сюда подключаются игроки, обязан быть открыт);
#   * externalPublicHost — адрес для КЛИЕНТА: с 127.0.0.1 голоса не будет.
echo -e "${CYAN}[4b/5] Настройка голосового чата...${NC}"

# Секрет — целое число: alt:V ожидает здесь именно число, строка в кавычках
# не подходит.
VOICE_SECRET=$(( (RANDOM << 15 | RANDOM) & 0x7FFFFFFF ))
VOICE_PORT="${FLOVMP_VOICE_PORT:-7896}"
VOICE_PUBLIC_PORT="${FLOVMP_VOICE_PUBLIC_PORT:-7895}"
...
if [ ! -x "${VOICE_DIR}/altv-voice-server" ]; then
  echo -e "${YELLOW}  Голосовой сервер не найден в ${VOICE_DIR} — голос пропущен.${NC}"
elif [ -z "${PUBLIC_HOST}" ]; then
  echo -e "${YELLOW}  Не удалось определить внешний адрес — голос пропущен.${NC}"
  echo -e "${YELLOW}  Задайте FLOVMP_PUBLIC_HOST=<ip|домен> и запустите установщик заново.${NC}"
else
  # Конфиг голосового сервера (читается из его рабочей папки).
  # playerHost именно 0.0.0.0: за NAT публичного IP на интерфейсе нет.
  cat > "${VOICE_DIR}/voice.toml" <<VOICECFG
# Внешний голосовой сервер alt:V. secret обязан совпадать с externalSecret
# в server.toml игрового сервера.
secret = ${VOICE_SECRET}
host = "0.0.0.0"
port = ${VOICE_PORT}
playerHost = "0.0.0.0"
playerPort = ${VOICE_PUBLIC_PORT}
loopback = 0
VOICECFG
  chmod 600 "${VOICE_DIR}/voice.toml"

  # Секция [voice] игрового сервера. Заменяем целиком, если она уже есть:
  # иначе повторная установка накопила бы дубли и сервер не стартовал.
  python3 - "${INSTALL_DIR}/server.toml" "${VOICE_PORT}" "${VOICE_PUBLIC_PORT}" "${PUBLIC_HOST}" "${VOICE_SECRET}" <<'PYVOICE'
import io, re, sys
path, port, public_port, public_host, secret = sys.argv[1:6]
...
text += "externalPublicPort = %s\n" % public_port
io.open(path, "w", encoding="utf-8", newline="\n").write(text)
print("  [voice] прописан: игрокам %s:%s" % (public_host, public_port))
PYVOICE

  echo -e "${GREEN}  Голос настроен.${NC}"
  echo -e "${YELLOW}  Откройте порт ${VOICE_PUBLIC_PORT} (UDP и TCP) в файрволе — иначе голоса не будет.${NC}"
fi
