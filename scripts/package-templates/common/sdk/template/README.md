...
Одновременно разрешено до 12 browser. Общая площадь CEF ограничена эквивалентом
12 полноэкранных слоёв 1920×1080. На 4K или при большом числе слоёв внутреннее
разрешение пропорционально уменьшается и страница остаётся на весь экран; в F8
появляется предупреждение, а точные размеры видны в `mp.browsers.stats`.
Объединяйте постоянный HUD в одну страницу и скрывайте неиспользуемые меню — это
сохраняет чёткость и снижает CPU/RAM/VRAM.

`mp.trigger` ограничен 240 событиями в секунду на browser (и 1000 суммарно):
это не мешает обычному HUD и вводу, но защищает игровой тик от случайного
