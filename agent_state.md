...
## Архитектура продукта (зафиксировано 2026-08-30)

Три отдельных продукта, ни один не называется "FloV:MP Launcher":
- **FloV:MP** — движок-мультиплеер (невидим игроку, работает под капотом)
- **Florida V Launcher** — единственная дверь для игрока (WPF .exe, устанавливает FloV:MP в AppData)
- **Florida V** — сам RP-сервер (российская тематика, временное название пока проект не назван)

## Курс на мультиплатформенную поддержку (Legacy b3889+ и Enhanced) (2026-08-30)

Принято решение о полной независимости FloV:MP от закрытого legacy-рантайма:
- Поддержка актуальных версий GTA V: **Legacy 1.0.3889.0+** и **Enhanced 1.0.1158.13+**.
- Модульная система профилей клиента (`ProfileService` в `FloVMP.Launcher.Core`, 26/26 тестов).
- Внедрен `SkinPatcher` для автоматической модификации `skin.bin` (подстановка SHA-256 URL NUI-оболочки перед запуском клиента).
- C#-гейммод `FloVMP.Core` (46/46 тестов) и NUI-клиент готовы к работе.
