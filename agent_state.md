...
Updated: 2026-09-13 (Автономная Pre-DB админ-система, Zero-Vulnerability аудит, 302 теста, SayonaraRP NoClip & ESP)

## Актуальный прогресс (2026-09-13)

- **Автономная Pre-DB админ-система (Развертывание без базы данных):**
  - Разработан и интегрирован `AdminBootstrapManager` (`FloVMP.Core.Admin`) с персистентным хранением в `config/admins.json`.
  - Поддержка управления сервером из системной консоли/терминала (`Alt.OnConsoleCommand`): `setadmin <id|ник|SC> <0-8>`, `setfounder <id>`, `adminlist`, `reloadadmins`, `claimtoken`, `say`, `kick`, `online`.
  - Одноразовый токен первичной настройки (`setupToken`): вывод в терминал при первом запуске, активация командой в игре `/claimowner <токен>` с выдачей 8 ранга (Основатель).
  - Авто-клейм первого игрока: при чистом запуске первый вошедший игрок автоматически закрепляется как Основатель в `config/admins.json`.
  - Строгий RBAC: команда `/alogin <пароль>` доступна ТОЛЬКО пользователям, имеющим подтвержденные права в конфиге/БД (`GetAssignedAdminRank > 0`), попытки взлома отклоняются на сервере.
- **Интеграция NoClip и ESP из SayonaraRP:**
  - **NoClip (Свободный полёт):** Точная сферическая векторная кинематика (`dirX = -sin(heading)*cos(pitch)`, `dirY = cos(heading)*cos(pitch)`, `dirZ = sin(pitch)`). 4 динамические передачи (Точная 0.18, Обычная 0.85, Спринт 3.5, Турбо 7.5). Мягкое опускание на землю при выходе (`setPlayerToGround`), если высота над ближайшей поверхностью $\le 20$ метров. Если выше 20м — персонаж остаётся в воздухе.
  - **ESP (Wallhack / Информационные теги):** Нативный рендеринг native text в GTA V. 4 режима (F3: `ВЫКЛ`, `Игроки`, `Транспорт`, `Все`).
    - Игроки: Head Bone (12844), дистанционное масштабирование текста, плавное затухание (fadeAlpha), отображение HP, AR, Дистанции. Линия оружия скрыта, если в руках кулаки или пусто.
    - Иерархия: Младшие админы (< 8) не видят Основателя (8) на ESP; Основатель видит всех. У себя при NoClip тег не дрожит (устранен лаг расчета костей).
- **Очистка репозитория от тестового RP:** удалены папки `auth/`, `hud/`, `inventory/`, `settings/`. Владелец получает чистый движок, чат и F8 консоль.
- **Синхронизация, тесты и деплой:**
  - 302 из 302 тестов пройдены успешно (261 Core + 41 Launcher).
  - 100% Brand Clean: 378 файлов проекта и 294 файла SDK проверены сканером (0 маркеров).
  - Дистрибутив `flovmp-scaffold.zip` (70.78 МБ) пересобран на CDN VDS, служба `flovmp.service` активна на портах 7788 (UDP/TCP).
