...
Updated: 2026-09-13 (Чистый движок без RP-мусора, ванильный стартер, интеграция NoClip и ESP из SayonaraRP)

## Актуальный прогресс (2026-09-13)

- **Интеграция NoClip и ESP из SayonaraRP:**
  - **NoClip (Свободный полёт):** Точная сферическая векторная математика (`dirX = -sin(heading)*cos(pitch)`, `dirY = cos(heading)*cos(pitch)`, `dirZ = sin(pitch)`). 4 динамические передачи: Точная (0.18, RMB/Alt), Обычная (0.85, WASD), Спринт (3.5, Shift), Турбо (7.5, LMB). Плавная кривая набора скорости (`* 1.025` до 8.0). Мягкое опускание на землю при выходе (`setPlayerToGround`), если не зажат Пробел.
  - **ESP (Wallhack / Информационные теги):** Аппаратный рендеринг native text в GTA V. 4 режима переключения клавишей **F3** (`ВЫКЛ`, `Игроки`, `Транспорт`, `Все`).
    - Игроки: Head Bone (12844), дистанционное масштабирование текста, плавное затухание прозрачности (fadeAlpha), отображение HP, AR, Дистанции, актуального оружия в руках.
    - Фикс багов SayonaraRP: устранено дрожание тега в NoClip у себя (отключение расчета костей для self в пользу координат камеры). Иерархия администраторов: скрытие старших админов от младших.
    - Транспорт: модель, номерной знак, HP двигателя, скорость в км/ч, дистанция.
- **Очистка репозитория от тестового RP:** удалены папки `auth/`, `hud/`, `inventory/`, `settings/`. Покупатель сервера получает только чистые файлы движка, чат и F8 консоль.
- **Админ-система и безопасность:** команда `/alogin <пароль>` с валидацией наличия прав, `/revive [id]`, `/setdim`, F5 быстрый телепорт TPM.
- **Синхронизация и деплой:** SDK синхронизирован, дистрибутив `flovmp-scaffold.zip` (70.76 МБ) пересобран на CDN VDS, служба `flovmp.service` активна.
