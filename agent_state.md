...
### 2026-09-25 — пункт 23 (асинхронный SDK), живой стенд в облаке

- Живой стенд alt:V в облачном контейнере: IPv6 в ядре нет, движок падал
  «Failed to create host» — обход LD_PRELOAD-прослойкой (AF_INET6 → AF_INET,
  адреса ::ffff:a.b.c.d), только для теста, в пакеты не входит. Ресурсы
  грузятся, консольные команды (stop/start) — через stdin.
- Проба Alt.Export/Import между C#-ресурсами (для пункта 24): синхронны —
  результат в том же вызове, тот же поток и тик, медленный экспорт держит
  вызывающего, исключение доходит до вызывающего, вызов ≈ прямой делегат.
  Import до загрузки экспортёра и несуществующего — БРОСАЕТ
  InvalidImportException (не false); не пойманное в OnStart роняет весь
  сервер. Alt.Emit — отложенный (как в 7d932ab).
...
- `VERSION` = 1.0.6-beta; описание — `docs/releases/1.0.6-beta.md` (всё после
  сборки 1.0.5-beta; четыре ночных коммита 23→24.09 — снайпер, очередь
  снимка, броня, журнал F12 — в 1.0.5-beta не попали: в её ASI нет строки
  «клавиша Rockstar перехвачена»).
- Выпуск одной командой на ПК владельца: `python scripts/release.py` —
  тесты → пакеты → брендинг → подпись → проверка распаковки (SHA-256 всех
  файлов по manifest.txt) → GitHub latest → README и flovmp-setup.zip на
  главной репозитория релизов → VDS (если есть REDL_TOKEN) → проверка, что
  releases/latest отдаёт новую версию. Источник README —
  `scripts/distribution/releases-repo/README.md`.
- Linux: у `get.sh` не было режима GitHub (только VDS, где большие файлы
  обрываются). Добавлены `--github owner/repo` и `--tag`, `curl -L`; в пакете
  `update.sh` (тот же загрузчик), источник запоминается в
  `config/update.env` — обновление `sudo bash /opt/flovmp/update.sh`.
  Проверено в контейнере: чистая установка, 1.0.5-beta → 1.0.6-beta с
  сохранением настроек, повторный запуск «не требуется», подмена описания
  релиза отклоняется.
- Найдено и исправлено: вторая новая установка на той же машине с именами
  базы по умолчанию меняла пароль общего пользователя `flovmp` (ALTER USER) —
  первый сервер терял базу. Теперь новая установка останавливается до любых
  изменений и подсказывает `--db-name/--db-user/--service/--port`.
- Установщик предупреждает о выключенном IPv6 (alt:V падает с «Failed to
  create host»; так и в облачном контейнере, поэтому живой старт сервера
  здесь не проверить).
- `publish_release_vds.py` обновляет и загрузчики в `/var/www/cdn/` (раньше
  `flovmp-license publish` менял только релиз, VDS раздавал старый get.sh) и
  сверяет текущую версию. `make_setup_zip.py --github` — архив установки
  сразу берёт релиз с GitHub.
- `natives.h`: все 286 хэшей сверены с публичной базой нативов
  (alloc8or/gta5-nativedb-data) — перегенерация по SDK перед сборкой ASI
  больше не нужна; 11 нативов пункта 9 и GET_VEHICLE_BODY_HEALTH совпали по
  хэшу, числу параметров и типу результата.
- В релиз вошла работа владельца с ПК, не выложенная вовремя: 11 коммитов
  загрузочного экрана (слиты в `3fdd912`) и незакоммиченные правки из stash —
  фон/логотип по ссылке (`loading.background_url`/`logo_url`, `http.cpp` на
  WinHTTP). Исправлено: ссылка теряла `?query` (CDN/Discord отвечали 403/404).
  Ветки `wip-stash-0/1/2`, `wip-http` на GitHub — временные, можно удалить:
  0 совпадает с master, 2 уже в master, 1 и http перенесены.
- VDS из облака недоступен (таймаут, нет REDL_TOKEN) — публикация на VDS
  только скриптом у владельца.
...
- Уточнение владельца: выход из машины её не удаляет; машины `/car`/геймода
  не убираются никогда (как alt:V/RAGE:MP), брошенный трафик — только пустой
  и без игроков рядом TTL секунд (как GTA/FiveM).
...
- 8b — клиент (`game.cpp`): VADD/VSTATE/VDEL/VOWN/VREG/VREJ/VSET, VREQ/VSYNC/
  VENTER/VLEAVE; общая `SteerVehicle` для PSTATE и VSTATE; натив
...
- 8d — сохранение: `VehicleStore.cs` (MariaDB/JSON, фоновая очередь),
  миграция 003 (`world`, `id`), восстановление с прежними ID; проверено на
  MariaDB 10.11 в контейнере. Тесты 526.
- Пункт 7 — античит, вторая линия: `SuspicionLedger` (веса, затухание,
  пороги notify/kick), `NativeAntiCheat` (списки оружия/моделей/машин, учёт
  патронов `WeaponLedger`, `ClockDriftDetector` по PING), обвязка —
  `StarterResource.AntiCheatLedger.cs`; все прежние проверки пишут в журнал.
  Найдено: в чёрном списке оружия 3 хэша были с опечатками (railgun,
  up-n-atomizer, EMP) — исправлено. Кик по умолчанию выключен. Тесты 544.
- Пункт 10 — сохранение игрока: `FloVMP.Core/Players/PlayerPersistence.cs`
  (MariaDB/JSON, состояния в памяти, данные геймода грузятся в фоне),
  миграция 004, обвязка `StarterResource.Players.cs`; общий фоновый писатель
...
- Пункт 11 — метрики и оповещения: `Diagnostics/ServerMetrics.cs`
  ...
  Тесты 566.
- Пункт 9 — интерьеры: виды мира `IPL` и `ISET` (сообщения `WIPL`, `WISET`,
  снимок и `WDEL` — общим путём), клиент `world.cpp` запоминает исходное
  состояние и возвращает его при отключении; `world.mp_map` → `ON_ENTER_MP`.
  11 нативов добавлены в `gen_natives.py` (имена SDK 2016) и вручную в
  `natives.h` — перед сборкой ASI перегенерировать по SDK.
- Повторный аудит (b6a223b..HEAD, code-review high) — 7 находок, все исправлены
  с тестами, которые падают на старом коде: быстрый перезаход и удаление во
  время загрузки (слой неподтверждённых записей в PlayerPersistence), 0
  патронов → «без учёта» (`WeaponLedger.Restore`), регистр ключей в базе
  (миграция 005), невидимые машины 1.0.6 вне реестра (`InRegistryVehicle`),
  `alerts.hang_sec` не доходил до сторожа, IPL при reloadmaps, событие
  уничтожения от команды сервера. `players.restore_weapons` по умолчанию off.
  Второй проход по исправлениям — ещё 2: опоздавшее чтение прежней загрузки
  (номер загрузки), IPL из кода ресурса считался картовым. Тесты 571.
...
- Однокомандная Linux-распаковка сначала проверяет имена и типы всех tar-членов,
  ...
  экспорт source-kit добавляет `SOURCE-MANIFEST.json`, переносимый
  `scripts/verify_source.py` и `.sha256` рядом с ZIP.
...
- Проверен альтернативный Steam-путь b3521: staging неполный (1145 файлов / 55,6
  ГиБ, отсутствуют 200 путей). Исправлено применение `--platform`: явный
  Steam-профиль теперь управляет и `SteamAppId`, и запуском, даже если staging
  содержит EGS-файлы. `271591` — лишь малый depot исполняемых файлов;
  для E2E нужен полный content depot `271594` по manifest
  `6991706980502578342`. DepotDownloader требует интерактивный пароль Steam и
  Steam Guard владельца, пароль в проект и в чат не сохраняется.
