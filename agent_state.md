...
## Установка карты Москвы (RMRP 2025) для проекта Держава Онлайн (2026-09-05)

- **Карта:** `C:\Users\User\Downloads\RMRP MAP 2025` (11.33 ГБ, 109 DLC-паков).
- **Оптимизация веса:** папки одежды `rmrp_clothes_*` (22 пака, 3.0 ГБ) исключены для Phase 1 (сохранено место на VDS 20 ГБ, скачивание ускорено в 3 раза).
- **Сжатый архив:** `dist/cdn/moscow_map.zip` (~4.5 ГБ) с CDN-манифестом `manifest-map.json`.
- **C# Гейммод:** `SpawnPoints.cs` дополнен координатами ключевых локаций Москвы (`MoscowRedSquare = (-262.0, -955.0, 31.5)`, `MoscowCity`, `MoscowPolice`, `MoscowHospital`). Точка спавна по умолчанию установлена в `MoscowRedSquare`. Тесты 46/46 пройдены.
- **Клиент:** `client/resources/flovmp-client/client/index.js` обновлён:
  - Добавлена безопасная загрузка коллизий `loadCollisionAndUnfreeze` (`requestCollisionAtCoord` + `getGroundZFor3dCoord` + `hasCollisionLoadedAroundEntity`) с защитой от провала сквозь текстуры.
  - Камера авторизации переведена на вид Красной площади и Кремля.
- **Linux-сервер:** создан скрипт `scripts/assemble-linux-server.ps1`, собран автономный пакет `dist/linux-server` (167 МБ) и архив `dist/flovmp-server-linux.tar.gz` (55 МБ) для Ubuntu 22.04 LTS (VDS `188.127.229.224`). Sentry-телеметрия нейтрализована.
- **Параллельная работа:** файлы лаунчера не трогаются, Клод работает над дизайном модалок загрузчика в стиле Majestic.
