...
- **Открыто**: авторизация лаунчера на проде (nginx → :7799, слушателя нет — решение
  владельца); прод поставлен не пакетом (нет manifest.txt); debug=true проверять живым
  клиентом; уязвимости портала (web); нагрузочный стенд.
