...
и передачу, экспортёр принимает его из `--altv-backup` вместе с
`--include-altv-engine`; после распаковки выгрузки разрешённый runtime можно
положить в локальную `engine/` перед запуском `pack_server.py`. Сборщик
переименовывает бинарники и патчит модуль C# под имя хоста `FloV.Net.Host`.
Без разрешённого runtime можно собирать и тестировать собственный код, но
нельзя выпустить готовый серверный архив.
