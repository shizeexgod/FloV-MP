...
> **Статус:** серверная установка, локальная подпись и централизованный
> lease/revocation реализованы; desktop dashboard остаётся в плане.
...
или чужой runtime. `license.flv`, `license.lease` и `config/flovmp.env` не
входят в общий пакет: установщик получает лицензию по ключу или владелец кладёт
выданный файл сам. Это не позволяет обновлению затереть настройки клиента;
`license.lease` создаётся runtime после успешной online-проверки и сохраняется
при обновлении/очистке установки.
