...
3. **Портативность (White-Label Export):** Владелец конкретного RP-проекта (например, «Держава Онлайн») может экспортировать фронтенд-модуль как отдельное Next.js / React-приложение или подключить веб-компонент к сайту своего проекта (`admin.project-name.ru`), указав `FLOVMP_SERVER_API_KEY` и адрес API.
