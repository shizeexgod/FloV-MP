...
Три проекта:
- `src/FloVMP.Launcher.Core` — вся логика без WPF (детект GTA, ядро клиента,
  статус сервера, настройки, CDN-манифест). Тестируемо.
- `src/FloVMP.Launcher` — WPF-UI, ссылается на Core.
- `tests/FloVMP.Launcher.Tests` — xUnit (сейчас: CDN-синхронизация, 6 тестов).
