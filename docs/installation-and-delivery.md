...
Игровой клиент Legacy b3889 подключается через встроенный `client-b3889` и
`connect.cmd`: пакет ставит ASI/native adapter и записывает адрес шлюза
(`порт игры + 10`). Отсутствие launcher/web в серверной поставке является
намеренным и не мешает серверу работать локально или на VDS.
