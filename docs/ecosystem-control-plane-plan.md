...
3. Сервер клиента при следующем онлайн-проверочном запросе получает отказ и
   переходит в ограниченный режим или останавливает production согласно
   договору тарифа.
...
Рекомендуемая политика: подписанный lease действителен 72 часа, сервер
проверяет его каждые 12 часов и сохраняет последний валидный lease локально.
При недоступности backend сервер продолжает работу до конца grace-period, но
после явного `revoked` новый lease не выдаётся. Для критичных нарушений можно
сократить grace только для конкретного ключа.
...
2. Сервер с валидным cached lease переживает краткий сбой backend в рамках grace.
3. После истечения grace или явного policy-revoke сервер предсказуемо меняет
   режим согласно тарифу, без потери пользовательских данных.
