...
CREATE TABLE IF NOT EXISTS `portal_servers` (
  ...
  `environment` VARCHAR(32) NOT NULL DEFAULT 'production' COMMENT 'production, development, test, staging',
  `name` VARCHAR(128) NOT NULL,
  `label` VARCHAR(64) DEFAULT NULL COMMENT 'Визуальный префикс в ЛК (напр. "Основной", "Тест PvP", "Стейдж 2")',
  `ip` VARCHAR(45) NOT NULL DEFAULT '127.0.0.1',
  `port` INT UNSIGNED NOT NULL DEFAULT 7788,
  `slot_limit` INT UNSIGNED DEFAULT NULL COMMENT 'Лимит слотов на сервер (NULL = без ограничения, берётся из лицензии проекта)',
  ...
  `notes` TEXT DEFAULT NULL COMMENT 'Заметки владельца по серверу',
  `created_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
