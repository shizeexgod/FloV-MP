...
-- ВАЖНО: ни CREATE DATABASE, ни USE здесь быть не должно.
-- MigrationRunner подключается уже к нужной базе (её имя берётся из
-- FLOVMP_DB_NAME / строки подключения) и сам создаёт её, если не существует.
-- Захардкоженное `flovmp_rp` увело бы миграции в чужую базу, а сервер
-- продолжил бы работать с пустой.
...
CREATE TABLE IF NOT EXISTS `accounts` (
  ...
  `jail_until_utc` DATETIME DEFAULT NULL,
